...
namespace FnSync
{
    public class PhoneMessageCenter : QueuedAsyncTask<PhoneMessageCenter.ControlDescriptor, object?>
    {
        ...
        public static readonly PhoneMessageCenter Singleton = new();

        private static readonly string IGNORED_MESSAGE = "IGNORED_MESSAGE";
        ...
        private class ActionDescriptor
        {
            ...
            public ActionDescriptor(MsgAction action, bool onMainThread, int order = 0)
            {
                this.action = action;
        ...
        private readonly HashSet<ActionDescriptor> Universals = new();
        private readonly Dictionary<Condition, HashSet<ActionDescriptor>> AllTypes = new();
        private readonly Dictionary<Condition, HashSet<ActionDescriptor>> AllIds = new();
        private readonly Dictionary<Condition, HashSet<ActionDescriptor>> Specifics = new();
        ...
        public void Register(string? id, string? msgType, MsgAction action, bool OnMainThread, int Order = 0)
        {
            ...
            this.InputManually(new ControlDescriptor(
                new ControlDescriptor.AdditionDescriptor(id, msgType, null, action, OnMainThread, Order)
                ));
        }
        ...
        public void Register(string id, string[] msgTypes, MsgAction action, bool OnMainThread, int Order = 0)
        {
            ...
            this.InputManually(new ControlDescriptor(
                new ControlDescriptor.AdditionDescriptor(id, IGNORED_MESSAGE, msgTypes, action, OnMainThread, Order)
                ));
        }
        ...
        private void Register(string? MsgType, ControlDescriptor.AdditionDescriptor Descriptor)
        {
            ...
            Condition condition = new(MsgType, Descriptor.id);
        ...
        public void Unregister(string? id, string? msgType, MsgAction action)
        {
            ...
            this.InputManually(new ControlDescriptor(
                new ControlDescriptor.DeletionDescriptor(id, msgType, null, action)
                ));
        }
        ...
        public async Task<JObject> OneShotMsgPart(PhoneClient Client, JObject Msg, string MsgType, byte[]? binary, string ExpectedType, int TimeoutMillseconds)
        {
            ...
            else if (msgObj is IMessageWithBinary mwb && mwb.Message is JObject message)
            {
                return message;
            }
            else
        ...
        private static void InvockOnMainThreadConditionly(
            bool On,
            ...
            )
        {
            ...
            AutoDisposableTimer FireAndCleanUp = new(null, TimeoutMillseconds, null, false);
            ...
            void MessageCallbackWrapper(string _, string ReceivedType, object? ReceivedMsgObj, PhoneClient? __)
            {
                ...
                if (ReceivedMsgObj is JObject msgObj)
                {
                    FinalMsg = msgObj;
                }
                else if (ReceivedMsgObj is IMessageWithBinary mwb)
                {
                    FinalMsg = mwb.Message;
                    FinalBinary = mwb.Binary;
                }
                else
                {
                    return;
        ...
        public Task<object> WaitForMessage(string id, string MessageType, int TimeoutMillseconds, Func<object?, bool>? Callback = null, int Count = 1, CancellationToken? Cancellation = null)
        {
            ...
            AutoDisposableTimer FireAndCleanUp = new(null, TimeoutMillseconds, Cancellation, false);
        ...
        public Task<PhoneClient> WaitOnline(string id, int TimeoutMillseconds, CancellationToken? Cancellation = null)
        {
            ...
            TaskCompletionSource<PhoneClient> completionSource = new();
        ...
        {
            public static readonly ActionDescriptorPriority Comparer = new();
            public int Compare(ActionDescriptor? x, ActionDescriptor? y)
            {
                int xOrder = x?.Order ?? int.MinValue;
                int yOrder = y?.Order ?? int.MinValue;
                int OrderCompare = xOrder.CompareTo(yOrder);

                int xActionHash = x?.action.GetHashCode() ?? int.MinValue;
                int yActionHash = y?.action.GetHashCode() ?? int.MinValue;
                int ActionCompare = Math.Sign(xActionHash - yActionHash);
        ...
        private void Raise(ControlDescriptor.RaisingDescriptor Descriptor)
        {
            ...
            {
                lookUpType = (string?)jmsg["intention"] ?? throw new ArgumentException("", "intention");
            }
            else
            {
            ...
            List<ActionDescriptor> descriptors = new();

            Condition specific = new(lookUpType, Descriptor.id);
            if (Specifics.ContainsKey(specific))
                descriptors.AddRange(Specifics[specific]);

            Condition allType = new(null, Descriptor.id);
            if (AllTypes.ContainsKey(allType))
                descriptors.AddRange(AllTypes[allType]);

            Condition allId = new(lookUpType, null);
